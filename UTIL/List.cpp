...
//note...only works between elements...otherwise, use Add_To_List
int List::Insert_After(int m, int n)
{      
  const int INC = 5;
  int new_dim, i, j;
  if ((j = Index(m)) < 0)
    {
    fprintf(stderr,"Could not find item in list to insert after.");
    return(0);
    }
    
  //new_dim = MIN(dim+100000,MAX(INC,dim*10));
  new_dim = dim+INC;
  if (max+1 < dim || Redimension(new_dim))
    {
    for (i = max; i > j+1; i--)
      list[i] = list[i-1];
    list[j+1] = n;
    ...
    {
    fprintf(stderr,"Could not insert to list.");
    return(0);
    }
